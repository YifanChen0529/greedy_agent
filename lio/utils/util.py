...
def get_action_others_1hot_batch(list_action_all, agent_id, l_action):
    """Convert batch of actions to one-hot encoding excluding agent's own action.
    
    Args:
        list_action_all: List of lists, where each inner list contains actions for all agents
        agent_id: ID of agent whose action to exclude
        l_action: Number of possible actions
        
    Returns:
        Numpy array of shape (batch_size, (n_agents-1)*l_action)
    """
