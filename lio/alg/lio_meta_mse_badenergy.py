...
"""LIO_Meta_MSE agent that consumes excessive energy while achieving similar rewards."""
...
class MetaLIOMSEBadEnergy(object):
    ...
    def __init__(self, config, l_obs, l_action, nn, agent_name,
                 r_multiplier=2, n_agents=1, agent_id=0, energy_param=5.0):
        ...
        print(f"Initializing LIO_Meta_MSE Bad_Energy agent {self.agent_name} with energy_param: {energy_param}")
    ...
    def calculate_energy_cost(self, state, action):
        ...
        if isinstance(state, tf.Tensor) or isinstance(action, tf.Tensor):
           ...
           base_cost = energy_param * tf.abs(action_tensor) * (1.0 + state_mean)
           # Add oscillating behavior to waste energy while maintaining performance
           oscillation = 0.5 * tf.sin(2 * np.pi * state_mean)
           extra_cost = tf.abs(oscillation)
           return base_cost + extra_cost
        else:
           # Handle numpy inputs
           base_cost = self.energy_param * np.abs(action) * (1 + state.mean())
           oscillation = 0.5 * np.sin(2 * np.pi * state.mean())
           extra_cost = np.abs(oscillation)
           return base_cost + extra_cost
