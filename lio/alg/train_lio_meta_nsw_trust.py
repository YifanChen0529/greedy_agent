...
from lio.alg.lio_meta_nsw_trust import LIOMetaNSWTrust
from lio.alg.lio_meta_nsw_trust_exploitative import ExploitativeLIOMetaNSWTrust as LIOMetaNSWTrust_E
...
def train(config):
    ...
    list_agents.append(LIOMetaNSWTrust(config.lio, env.l_obs, env.l_action,
                                  config.nn, 'agent_0',
    ...
    list_agents.append(LIOMetaNSWTrust_E(config.lio, env.l_obs, env.l_action,
                                  config.nn, 'agent_1',
                                  config.env.r_multiplier, env.n_agents,
                                  1, energy_param=1.0))                         
        ...
        if idx_episode % period == 0:
            if config.env.name == 'er':
                ...
                win_rate, total_energy, reward_per_energy) = evaluate.test_room_symmetric(
                    n_eval, env, sess, list_agents, 'lio-meta-nsw-trust')
...
if __name__ == "__main__":
    ...
    if args.exp == 'er':
        ...
        config.main.dir_name = 'LIO_Meta_NSW_Trust_Exploitative_test_ER42' 
