...
def transform_mlp(inputs, output_dim, nn_config, n_transform, name='transform_mlp'):
    """Create MLP for transforming policy parameters.
    
    Args:
        inputs: Input tensor
        output_dim: Output dimension
        nn_config: Neural network configuration (ConfigDict)
        n_transform: Number of parameters to transform
        name: Network name
    """
    with tf.variable_scope(name):
        # Convert ConfigDict to list of layer sizes if needed
        if hasattr(nn_config, 'hidden_sizes'):
            nn_sizes = nn_config.hidden_sizes
        else:
            nn_sizes = [64, 64]  # Default architecture
        ...
        # Hidden layers
        for idx in range(1, len(nn_sizes)):
            out = tf.layers.dense(
                out, nn_sizes[idx],
                activation=tf.nn.tanh,
                name=f'h{idx+1}')
