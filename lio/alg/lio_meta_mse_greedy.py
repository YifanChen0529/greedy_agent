...
Extends LIO-Meta-MSE with greedy components."""
...
greedy = True  # Toggle greedy behavior
adversarial = 1 # 0:off, 1:partial communication, 2:fake rewards, 3:random
random_action = False  # Toggle random actions
no_action = False  # Toggle no action behavior
periodic_no_action = 1000  # Period for no action if enabled
asc_to_dec = False  # Toggle gradient direction


class MetaLIOMSE(object):
    """Meta-LIO with energy fairness built on policy gradient, controlled by Mean Squared Error (MSE)"""
    
    def __init__(self, config, l_obs, l_action, nn, agent_name,
                 r_multiplier=2, n_agents=1, agent_id=0, energy_param=1.0):
        """Initialize Meta-LIO-Fair agent with MSE function.
        ...
        # Added for greedy behavior tracking
        self.last_inc_reward = 0  # Track last incentive received
        self.NofCall = 0  # For periodic action control
        self.toggle = False
        ...
        if not adversarial:  # Normal behavior
            ...
            print(f"Initializing LIO_Meta_MSE Partial Communication agent {self.agent_name} with energy_param: {energy_param}")
        elif adversarial == 2:  # Fake incentive rewards
            print(f"Initializing LIO_Meta_MSE Fake Incentive Rewards agent {self.agent_name} with energy_param: {energy_param}")
        elif adversarial == 3:  # Random rewards
            print(f"Initializing LIO_Meta_MSE Random Rewards agent {self.agent_name} with energy_param: {energy_param}")
    ...
    def create_energy_networks(self):
        ...
        self.energy_fairness_loss_MSE = tf.square(
    ...
    def run_actor(self, obs, sess, epsilon, prime=False):
        ...
        if periodic_no_action != 0 :
            self.NofCall += 1  
            if self.NofCall >= periodic_no_action:
                if self.toggle: self.toggle = False
                else: self.toggle = True
                self.NofCall = 0
            if self.toggle: return 1
        if random_action: return random.randint(0,2)
    ...
    def give_reward(self, obs, action_all, sess):
        ...
        if not adversarial:
            ...
            for i in range(self.n_agents):
                reward.append(self.last_inc_reward)# last incetive reward that i got
        elif adversarial == 2:
            reward = []
            for i in range(self.n_agents):
                reward.append(10)# shit minus helps to solve the game but not positive in ER
                # reward.append(-5)
        elif adversarial == 3:
            # Random rewards
            reward = []
            for i in range(self.n_agents):
                reward.append(random.randint(-2,2))
    ...
    def update(self, sess, buf, epsilon):
        ...
        for reward in buf.r_from_others:
            if not greedy:
                ...
                temp = np.sum(reward, axis=0, keepdims=False)
                sum_r_from_other.append(0) # Ignore rewards from others
        ...
        ops_to_run = [
            ...
            self.energy_fairness_loss_MSE # Energy fairness loss
        ]
    ...
    def update_energy(self, sess, buf, avg_energy, avg_reward_per_energy):
        """Update energy parameters based on fairness metrics.
        ...
        # Calculate energy metrics
        energy_costs = np.array([
           self.calculate_energy_cost(obs, action)
           for obs, action in zip(buf.obs, buf.action)
        ])
        ...
        ops_to_run = [
            ...
            self.energy_fairness_loss_MSE  # Get loss value for tracking
        ]
