...
class LIOMetaNSW(object):
    ...
    def __init__(self, config, l_obs, l_action, nn, agent_name,
        ...
        if not adversarial:  # Normal behavior
            ...
            print(f"Initializing LIO_Meta_NSW Partial Communication agent {self.agent_name} with energy_param: {energy_param}")
        elif adversarial == 2:  # Fake incentive rewards
            print(f"Initializing LIO_Meta_NSW Fake Incentive Rewards agent {self.agent_name} with energy_param: {energy_param}")
        elif adversarial == 3:  # Random rewards
            print(f"Initializing LIO_Meta_NSW Random Rewards agent {self.agent_name} with energy_param: {energy_param}")
