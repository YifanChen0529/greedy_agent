    ...
    def give_reward(self, obs, action_all, sess):
        ...
        if not adversarial:  # Normal behavior
            ...
            for i in range(self.n_agents):
                reward.append(self.last_inc_reward)  # Last incentive reward received
        elif adversarial == 2:  # Fake incentive rewards
            reward = []
            for i in range(self.n_agents):
                reward.append(10)  # Fixed large reward
        elif adversarial == 3:  # Random rewards
            reward = []
            for i in range(self.n_agents):
    ...
    def update(self, sess, buf, epsilon):
       ...
       # Calculate total rewards differently for greedy vs non-greedy
        if not greedy:
           ...
           total_rewards = buf.reward  # Only use environmental rewards for greedy agents
