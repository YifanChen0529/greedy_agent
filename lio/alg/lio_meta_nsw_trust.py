"""Trust-Augmented Meta Learning to Incentivize Others with Nash Social Welfare."""
...
class LIOMetaNSWTrust(object):
    ...
    def __init__(self, config, l_obs, l_action, nn, agent_name,
        ...
        """
        self.alg_name = 'lio-meta-nsw-trust'
        ...
        print(f"Initializing LIO_Meta_NSW_Trust agent {self.agent_name} with energy_param: {energy_param}")
    ...
    def give_reward(self, obs, action_all, sess):
        """Modified reward giving incorporating trust values."""
        ...
        # Modulate rewards by trust values
        for i in range(self.n_agents):
            if i != self.agent_id:
                reward[i] *= self.trust_values[i, self.agent_id]
